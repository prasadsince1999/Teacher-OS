...
abstract final class ChapterTransformer {
  static ChapterVM transform(ChapterJSON ch) {
    String subjectName;
    if (ch.subject.toLowerCase() == 'math' ||
        ch.subject.toLowerCase() == 'mathematics') {
      subjectName = 'Mathematics';
    } else {
      subjectName = 'Science';
    }

    final bookName = ch.book.isNotEmpty
        ? ch.book
        : (subjectName == 'Mathematics'
              ? (ch.grade >= 6
                    ? 'Ganita Prakash'
                    : (ch.grade == 5 ? 'Math-Magic' : 'Maths Mela'))
              : (ch.grade >= 6
                    ? 'Curiosity'
                    : (ch.grade == 5
                          ? 'Looking Around'
    ...
    return ChapterVM(
      ...
      subject: subjectName,
      book: bookName,
      chapterNumber: ch.chapterNumber > 0 ? ch.chapterNumber : 1,
      title: ch.title,
      progress: 0,
      last: '—',
      ...
      teacherBrief: _buildTeacherBrief(ch),
    );
  }
  ...
  static TeacherBriefVM _buildTeacherBrief(ChapterJSON ch) {
    final backgroundFacts = <String>[];
    final likelyQuestions = <LikelyQuestionVM>[];
    final misconceptions = <MisconceptionVM>[];
    final ifStuck = <String>{};
    final doNot = <String>{};
    final boardPlans = <String>[];

    for (final t in ch.topics) {
      if (t.commonConfusion != null) {
        misconceptions.add(
          MisconceptionVM(
            wrong: t.commonConfusion!.wrong,
            right: t.commonConfusion!.right,
            why: t.commonConfusion!.why,
          ),
        );
      }

      if (t.teacherBrief != null) {
        final b = t.teacherBrief!;
        backgroundFacts.addAll(b.backgroundForMe);
        for (final lq in b.likelyQuestions) {
          likelyQuestions.add(
            LikelyQuestionVM(q: lq.q, a: lq.a, depth: lq.depth),
          );
        }
        ifStuck.addAll(b.ifStuckSay);
        doNot.addAll(b.doNotSay);
        if (b.boardPlan != null && b.boardPlan!.isNotEmpty) {
          boardPlans.add(b.boardPlan!);
        }
      }
    }

    return TeacherBriefVM(
      inOneLine: ch.chapterIntro.isNotEmpty
          ? '${ch.chapterIntro.split('.').first}.'
          : 'People live and work together in interconnected communities.',
      whyItMatters: ch.outcomes.isNotEmpty ? ch.outcomes.first : 'Helps students understand how people and places in a community connect.',
      outcomes: ch.outcomes,
      backgroundForMe: backgroundFacts,
      likelyQuestions: likelyQuestions,
      misconceptions: misconceptions,
      ifStuckSay: ifStuck.toList(),
      doNotSay: doNot.toList(),
      boardPlan: boardPlans.isNotEmpty ? boardPlans.join('\n\n---\n\n') : null,
    );
  }
}
